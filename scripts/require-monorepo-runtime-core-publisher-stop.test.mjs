...
test('accepts an externally owned, non-deployable source mirror', () => {
  assert.doesNotThrow(() => assertMonorepoPublisherStopped(externalManifest(), skippedMirror));
});

test('accepts removal of the monorepo source mirror', () => {
  assert.doesNotThrow(() => assertMonorepoPublisherStopped(externalManifest(), undefined));
});
...
test('rejects the monorepo while it still lists runtime-core as public', () => {
  ...
  assert.throws(() => assertMonorepoPublisherStopped(manifest, skippedMirror), /still declares runtime-core as a public artifact/);
});

test('rejects a missing external ownership declaration', () => {
  assert.throws(() => assertMonorepoPublisherStopped({ publicArtifacts: [], externalArtifacts: [] }, skippedMirror), /does not declare runtime-core as externally owned/);
});
...
test('rejects a deployable monorepo mirror', () => {
  ...
  assert.throws(() => assertMonorepoPublisherStopped(externalManifest(), pom), /source mirror remains deployable/);
});
