...
base_repo = "The-Pipeline-Framework/pipelineframework-contracts"
coords = [
...
assert not meta_path.is_symlink(), "build metadata must not be a symlink"
assert metadata["schemaVersion"] == 1 and metadata["repository"] == base_repo
assert metadata["component"] == "contracts"
assert isinstance(metadata["sourceRepository"], str) and re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", metadata["sourceRepository"])
assert set(metadata["provenance"]) == {"build"}, "unexpected build provenance fields"
...
if event == "pull_request":
    ...
    assert head["repo"] is not None and head["repo"]["full_name"].lower() == metadata["sourceRepository"].lower()
