...
export function assertMonorepoPublisherStopped(manifest, mirrorPom) {
...
async function main() {
  ...
  const [manifestJson, mirrorPom] = await Promise.all([
    githubFile('framework/public-artifacts.json', commitSha),
    githubFile('framework/runtime-core/pom.xml', commitSha, true),
  ]);
  assertMonorepoPublisherStopped(JSON.parse(manifestJson), mirrorPom);
